...
HTML;
    ...
    private function state_save_config_file() {
        ...
        if ( ! is_writeable('config') ) {
            $e = new Exception('render config permissions issue');
            $this->stage_content = $this->format_write_config_result($e);
        }
        elseif ( ! is_writeable($config_path) ) {
            $e = new Exception("render $config_path permissions issue");
            $this->stage_content = $this->format_write_config_result($e);
        }
        else {
            ...
            $this->stage_content = $this->format_write_config_result($config_content);
        }
        $this->stage_content = sprintf('<pre>%s</pre>', $this->stage_content);
    ...
    private function format_config_file($config) {
        $documentation_link = 'http://docs.wikkawiki.org/ConfigurationOptions';
        $config_var = '$wakkaConfig';
        $glue = "\n    ";
...
XPHP;
        ...
        return sprintf($format,
            ...
            $documentation_link,
            $config_var,
            implode($glue, $config_lines)
        );
    }
    
    protected function format_write_config_result($content) {
        var_dump($content);
